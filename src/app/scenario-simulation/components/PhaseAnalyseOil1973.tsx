...
const oilPriceData = [
  { year: 'Jan 73', price: 3.0 },
  { year: 'Apr 73', price: 3.2 },
  { year: 'Jul 73', price: 3.5 },
  { year: 'Oct 73', price: 5.1 },
  { year: 'Jan 74', price: 8.4 },
  { year: 'Apr 74', price: 11.2 },
  { year: 'Jul 74', price: 12.0 },
  { year: 'Oct 74', price: 11.8 },
...
const CustomMacroTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 shadow-xl">
      <p className="text-xs text-zinc-400 mb-1">{label}</p>
      {payload.map((p: any) => (
        <p key={`tt-${p.dataKey}`} className="text-xs font-mono" style={{ color: p.color }}>
          {p.name}: {p.value}%
        </p>
      ))}
    </div>
                ...
                <div className="flex items-end justify-between">
                  ...
                  <svg viewBox="0 0 100 50" className="w-20 h-10 opacity-60">
                    <path
                      d={sparkPath}
                      fill="none"
                      stroke={ind.trend === 'up' ? '#f87171' : ind.trend === 'down' ? '#34d399' : '#6b7280'}
                      strokeWidth="2"
                      strokeLinecap="round"
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <AreaChart data={oilPriceData} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
              ...
              <Area type="monotone" dataKey="price" stroke="#f59e0b" strokeWidth={2} fill="url(#oilGrad)" dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
